...
control_lora = ControlLoRA.from_pretrained('trained/sd-sketch-fscoco-control-lora', subfolder="sd-controlora-rank4") #change this to my model to run custom models 
...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, sample_steps, scale, seed, eta, low_threshold, high_threshold):
    with torch.no_grad():
        ...
        control = torch.from_numpy(img[...,::-1].copy().transpose([2,0,1])).float().to(device)[None] / 127.5 - 1
